...
export default function EditCalculatorScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const router = useRouter();
  
  const { getCalculator, updateCalculator } = useCalculatorStore();
  ...
  const handleSave = () => {
    ...
    // Navigate directly to calculator page instead of showing an alert
    router.replace(`/calculator/${id}`);
  };
  
  return (
    <>
      <Stack.Screen options={{ title: 'Edit Calculator' }} />
